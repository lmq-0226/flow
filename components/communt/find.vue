<template>
	<view>
		<scroll-view :scroll-y="true" class="scroll-Y" :refresher-enabled="true" @refresherrefresh="getData()" :refresher-triggered="triggered">
			<view class="goods">
				<!-- 瀑布流 -->
				<u-waterfall v-model="flowList" ref="uWaterfall">
					<template v-slot:left="{leftList}">
						<view class="demo-warter" v-for="(item, index) in leftList" :key="index"
							...
							<image v-if="item.type == 'video'" class="videoPb" src="/static/comm/video_play.png"
								mode=""></image>
		...
		methods: {
			getData() {
				...
				this.request({
					...
					if (res.data.code == 1) {
						res.data.data.data.forEach(elem=>{
							elem.poster = this.ImgUrl + elem.images.join(',')
						})
