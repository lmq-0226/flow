				...
				<view class="list">
					<view class="item" v-for="(item,index) in identList" :key="item.id" @click="next(item)">
						...
						<view class="goods">
							...
							<view class="result">
								<image v-if="item.result == 2" src="/static/my/true.png" mode=""></image>
								<image v-else-if="item.result == 3" src="/static/my/fake.png" mode=""></image>
							</view>
						</view>
						<view class="bot">
							<text>{{item.time}}</text>
...
<script>
	export default {
		...
		methods: {
			...
			next(e) {
				this.go('/pages/my/identify/detail?id=' + e.id)
			},
			go(e) {
				uni.navigateTo({
					url: e
