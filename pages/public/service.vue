			...
			>
				<view v-for="(row, index) in msgList" :key="index" :id="'msg' + row.id">
					<view class="cu-item self" v-if="row.form.id == user_id">
						...
						<view class="cu-avatar round" :style="{ backgroundImage: 'url(' + $wanlshop.oss(ImgUrl + row.form.avatar, 44, 44, 2, 'avatar') + ')' }"></view>
					</view>

					<view class="cu-item" v-else>
						<view class="cu-avatar round" :style="{backgroundImage: 'url('+ $wanlshop.oss(ImgUrl + row.form.avatar) +')'}" v-if="row.form.avatar"></view>
						<view class="cu-avatar round" :style="{backgroundImage: 'url('+ ImgUrl + '/assets/addons/wanlshop/img/common/logo.png' +')'}" v-else></view>
						<!-- 文字消息 -->
						<view class="main" v-if="row.message.type == 'text'">
