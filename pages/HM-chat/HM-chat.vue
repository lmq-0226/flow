						...
						<view class="cu-item self" v-if="row.to_id == to_id">
							...
							<view class="main goods" v-if="row.message.type == 'goods'">
								<view class="content" @tap="onGoods(goods.id)">
									<image :src="$wanlshop.oss(ImgUrl + row.message.content.image, 100, 0)" mode="widthFix"></image>
								...
								<view class="content" @tap="orderDetails(row.message.content.id)">
									...
									<view class="margin-tb-bj radius product padding-sm">
										<view class="item" v-for="(item, index) in row.message.content.goods" :key="index">
											<view class="image">
												<image :src="$wanlshop.oss(ImgUrl + item.image, 50, 50)" mode="aspectFill"></image>
											</view>
							...
							<view class="cu-avatar radius" :style="{ backgroundImage: 'url(' + $wanlshop.oss(ImgUrl + row.form.avatar, 100, 100) + ')' }"></view>
...
export default {
	...
	onLoad(option) {
		...
		// 临时关闭推送
		this.$store.commit('chat/setIschat', {notice: true});
		...
		this.request({
			...
			if(res.data.code == 1){
				this.to_id = res.data.data.user_id;
				this.shop_id = res.data.data.id;
				// 新版新追加
				this.shop_name = res.data.data.shopname;
				this.shop_avatar = res.data.data.avatar;
				this.$wanlshop.title(res.data.data.shopname + (res.data.data.isOnline == 1 ? '-在线':'-离线'));
				// 初始界面
				this.getMsgList(res.data.data.user_id);
			}
		})
		//语音自然播放结束
		...
		loadHistory(e) {
			...
			this.request({
				...
				if(res.data.code == 1){
					...
					this.msgList = res.data.data.current_page == 1 ? list : list.concat(this.msgList);
					...
					if (res.data.data.current_page == res.data.data.last_page) {
						...
						this.current_page = res.data.data.current_page + 1;
					}
						...
						if(updata.data.code == 1){
							for (let i = 0; i < res.tempFilePaths.length; i++) {
								...
								uni.getImageInfo({
									...
									success: image => {
										uni.uploadFile({
											url: updata.data.data.uploadurl, //仅为示例，非真实的接口地址
											header:{
												token: uni.getStorageSync('userInfo').token
											},
											filePath: image.path,
											name: 'file',
											formData: updata.data.data.storage == 'local' ? null : updata.data.data.multipart,
											success: (data) => {
												let img_url = JSON.parse(data.data).data
												this.sendMsg({ url: img_url.fullurl, w: image.width, h: image.height }, 'img');
											}
										});
									}
								});
